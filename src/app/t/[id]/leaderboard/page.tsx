...
export default function LeaderboardPage() {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    if (!tournamentRes.ok || !scoresRes.ok) return;

    const tournament = await tournamentRes.json();
    const scores: PlayerScore[] = await scoresRes.json();

    const holes: CourseHole[] = tournament.courseHoles
      .map((h: { hole: number; par: number; hcp?: number | null }) => ({
        hole: h.hole,
        par: h.par,
        hcp: h.hcp || null,
      }))
    ...
    holes.forEach((h) => {
      coursePars[h.hole] = h.par;
    });

    const totalCourseHoles = holes.length || 18;

    const leaderboard: LeaderboardEntry[] = tournament.players.map(
      (player: {
        id: number;
        name: string;
        nickname: string | null;
        ...
        // Net calculation: use hole handicaps if available for proper stroke allocation
        const hasHoleHcps = holes.some((h) => h.hcp);
        let handicapStrokes: number;

        if (hasHoleHcps && player.handicap > 0) {
          // Allocate strokes to hardest holes first (lowest hcp number = hardest)
          // Player gets a stroke on holes where their handicap >= the hole's hcp ranking
          handicapStrokes = playerScores.reduce((strokes, s) => {
            const holeHcp = holes.find((h) => h.hole === s.hole)?.hcp;
            if (holeHcp && player.handicap >= holeHcp) {
              // Extra stroke for handicaps > 18 (second pass)
              const extraStroke = player.handicap >= holeHcp + 18 ? 2 : 1;
              return strokes + extraStroke;
            }
            return strokes;
          }, 0);
        } else {
          // Fallback: prorate handicap evenly across holes played
          handicapStrokes = Math.round(
            (player.handicap * holesCompleted) / totalCourseHoles
          );
        }

        const netStrokes = totalStrokes - handicapStrokes;
        const netToPar = netStrokes - parForPlayed;
  ...
  // Sort entries based on current view mode
    ...
    >
      ...
      {sortedEntries.length === 0 ? (
        ...
        <div className="overflow-hidden">
          ...
          <div className={cn(
            ...
            {viewMode === "net" && (
              <span className="py-2 text-center">Hcp</span>
            )}
          ...
          {sortedEntries.map((entry, index) => {
            ...
            return (
              ...
              >
                <span className="py-2.5 text-center text-sm font-semibold text-[#006747]">
                  {rank}
                </span>
                ...
                {viewMode === "net" && (
                  <span className="py-2.5 text-center text-xs text-[#006747]/50">
                    {entry.handicap}
                  </span>
                )}
                <span
      ...
      >
        <DialogContent className="bg-white border-[#006747]/20 max-w-[100vw] sm:max-w-lg text-[#006747] p-3 sm:p-4 rounded-lg sm:rounded-xl mx-1">
          <DialogHeader>
            <DialogTitle className="text-[#006747] text-base">
              Official Score Card
            </DialogTitle>
          </DialogHeader>
          ...
          {selectedPlayer && (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={cn(
                  "font-bold text-sm",
                  selectedPlayer.toPar < 0 ? "text-red-600" : selectedPlayer.toPar > 0 ? "text-[#333]" : "text-[#006747]"
                )}>
              ...
              <div className="flex items-center gap-3 text-[9px] text-[#006747]/70 flex-wrap">
                <span className="flex items-center gap-1">
                  <span className="w-3.5 h-3.5 rounded-full border-2 border-[#006747] inline-flex items-center justify-center">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#006747]" />
                  </span>
                  Eagle
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-3.5 h-3.5 rounded-full border-2 border-[#006747] inline-block" />
                  Birdie
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-3.5 h-3.5 border-2 border-[#006747] inline-block" />
                  Bogey
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-3.5 h-3.5 border-2 border-[#006747] bg-[#006747]/10 inline-block" />
                  D Bogey+
                </span>
              </div>
                            ...
                            {h.strokes !== null ? (
                              <div className="flex items-center justify-center">
                                {diff !== null && diff <= -2 ? (
                                  ...
                                  <span className="w-6 h-6 border-2 border-[#006747] bg-[#006747]/10 flex items-center justify-center text-[10px] font-bold text-[#006747]">{h.strokes}</span>
                                ) : (
                          ...
                          return (
                            <td key={h.hole} className="py-1 text-center">
                              {h.strokes !== null ? (
                                <div className="flex items-center justify-center">
                                  {diff !== null && diff <= -2 ? (
                                    <span className="w-6 h-6 rounded-full border-2 border-[#006747] flex items-center justify-center">
                                      <span className="w-4 h-4 rounded-full border border-[#006747] flex items-center justify-center text-[10px] font-bold text-[#006747]">{h.strokes}</span>
                                    </span>
                                  ) : diff === -1 ? (
                                    <span className="w-5 h-5 rounded-full border-2 border-[#006747] flex items-center justify-center text-[9px] font-bold text-[#006747]">{h.strokes}</span>
                                  ) : diff === 1 ? (
